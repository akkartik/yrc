...
(= threadlife* 30  requests* 0  timeouts* 0  requests/ip* (table)
  ...
  (withs ((i o ip)    (socket-accept s)
    ...
    (= th2 (thread "timeout"
             ...
             (unless (and th1 (dead th1))
               ...
               (++ timeouts*)
               (when (is 0 (remainder timeouts* 10))
                 (system date)
                 (prn timeouts* "/" requests* " timeouts")))
...
(def handle-request-3 (type i o op args n cooks ip t0)
  (let t1 (msec)
    (handle-request-4 type i o op args n cooks ip)
    (log-request type op args cooks ip t0 t1)))

(def handle-request-4 (type i o op args n cooks ip)
...
;; HACK for readwarp: every session gets a user cookie
;; make sure this runs after the http header is emitted in any path within
;; respond.
(mac ensure-user-cookie(cooks)
  `(when (blank:alref ,cooks "user")
     (let cook (unique-id)
       (push (list "user" cook) ,cooks)
...
(def new-bgthread (id f sec)
  ...
  (= (bgthreads* id) (new-thread "bgthread" (fn ()
                                   (while t
                                     (sleep sec)
                                     (f))))))
