...
(include "lib/defpat.arc")
(include "lib/treeparse.arc")
...
(w/nils (s clause forms)
  ...
  (mac ccond clauses (parse-all s clauses)))
...
(def uunion (< xs ys)
  ...
  (hcase `(,xs ,ys)
    ...
    () ys = ys
    (x . xt) (y . yt)
    || < x y = (cons x (uunion < xt ys))
    || < y x = (cons y (uunion < xs yt))
    || t     = (cons x (uunion < xt yt))))
