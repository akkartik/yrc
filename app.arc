...
(= hpwfile*   "snapshots/hpw"
   oidfile*   "snapshots/openids"
   adminfile* "snapshots/admins"
   cookfile*  "snapshots/cooks")
...
;; HACK for readwarp: every user cookie gets a user
(def ensure-cookie-has-user(req)
  (aif (alref req!cooks "user")
    (unless (cookie->user* sym.it)
      (cook-user (stringify:unique-id) sym.it))
    (ero "Error: no user cookie")))

(def get-user (req)
  (ensure-cookie-has-user req)
...
(def cook-user (user (o id))
  (or= id (new-user-cookie))
  (= (cookie->user*   id) user
     (user->cookie* user)   id)
...
(def bad-newacct (user pw)
  (if (no (goodname user 2 254))
       "Usernames can only contain letters, digits, dashes and
        underscores, and should be between 2 and 254 characters long.
